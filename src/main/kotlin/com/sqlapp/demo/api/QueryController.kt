...
data class AddQueryRequest(val query: String)
...
class QueryController(
    ...
    @PostMapping("/queries")
    fun add(@RequestBody body: AddQueryRequest): ResponseEntity<Any> {
        val id = repo.add(body.query)
    ...
    @GetMapping("/execute")
    fun execute(@RequestParam("query") id: Long): ResponseEntity<Any> {
        val sql = repo.findById(id) ?: return ResponseEntity.notFound().build()
        return try {
